...
#Action function
   ...
   #2 functions, init is called once while run is called by the timer
    def IrampeFunc(self):
        ...
        while True:
            if self.ShouldStop():
                action= self.SweepParam[1].split('Current')[0]
                action+= 'Pause'
       
                
                self.WriteCustom([self.SweepParam[0],action,self.SweepParam[2]])
                return()
            if self.i >= len(self.Lcons):
                if self.SweepParam[5] == 'inf':    
    ...
    def SaveDataFunc(self):
        ...
        with open(self.VariablesToStr(param[1]),'w', newline='', encoding='utf-8') as csvfile:
    ...
    def SaveDataLogFile(self,name):
        ...
        if len(namesplt) > 1:
            ...
            filename = ".".join(namesplt[:-1]) + '_log.' +  namesplt[-1] # Log files for filenames that had dots in the name were being cutted - now, the list generated by the function split is concatenated before adding the names

        else:
            filename = name + '_log'
        with open(filename,'w', newline='', encoding='utf-8') as csvfile: #trying to add encoding utf-8 to avoid errors while writing the file
            writer = csv.writer(csvfile, delimiter='\t', quotechar='|')
